// ==UserScript==
// @name              网盘直链下载助手（Plus）
// @namespace         https://www.intumu.com
// @version           1.0.0
// @author            wujukai123
// @icon              https://nd-static.bdstatic.com/m-static/v20-main/favicon-main.ico
// @icon64            https://nd-static.bdstatic.com/m-static/v20-main/favicon-main.ico
// @description       获取网盘文件真实下载地址。现已支持百度网盘、阿里云盘、天翼云盘、迅雷云盘、夸克网盘五大网盘，可使用Aria2下载，完美适配 Chrome，Edge，FireFox，360，QQ 等浏览器，可在无法安装客户端的环境下使用，助手免费开源。
// @license           AGPL-3.0-or-later
// @homepage          https://www.intumu.com
// @supportURL        https://pan.baidu.com/s/15HWGjk53UKxmRm-lUQc1Yw?pwd=id7c
...
(function () {
    ...
    let baidu = {
        ...
        addButton() {
            if (!pt) return;
            let $toolWrap;
            let $button = $(`<div class="g-dropdown-button pointer pl-button"><div style="color:#fff;background: ${color};border-color:${color}" class="g-button g-button-blue"><span class="g-button-right"><em class="icon icon-download"></em><span class="text" style="width: 60px;">下载助手</span></span></div>
			
			<div class="menu" style="width:auto;z-index:41;border-color:${color}">
			
			
			<div style="color:${color}" class="g-button-menu pl-button-mode" data-mode="rpc">选择文件下载</div>
            ...
            if (pt === 'main') {
				...
				<li class="sub cursor-p pl-button-mode" data-mode="rpc">选择文件下载</li>
        ...
        generateDom(list) {
            ...
            list.forEach((v, i) => {
                ...
                if (mode === 'rpc') {
                    content += `<div class="pl-item">
                                <div class="pl-item-name listener-tip" data-size="${size}">${filename}</div>
                                <button class="pl-item-link listener-link-rpc pl-btn-primary pl-btn-info" data-filename="${filename}" data-link="${dlink}"><em class="icon icon-device"></em><span style="margin-left: 5px;">推送到 Aria2 下载器</span></button></div>`;
                }
            });
            content += '</div>';
            if (mode === 'rpc') {
    ...
    let xunlei = {
        ...
        addButton() {
            if (!pt) return;
            let $toolWrap;
            let $button = $(`<div class="xunlei-button pl-button"><i class="xlpfont xlp-download"></i><span style="font-size: 13px;margin-left: 6px;">下载助手</span><ul class="pl-dropdown-menu" style="top: 34px;">
